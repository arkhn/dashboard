...
const browsers = browserNames.map(k => ({
  label: k,
  usage: browserUsage[0][k]
}));
        ...
        return (
          <svg width={w} height={h}>
            ...
            <Group top={centerY} left={centerX}>
                    ...
                    return (
                      <g key={`browser-${arc.data.name}-${i}`}>
                        <path
                          d={pie.path(arc)}
                          fill={color1}
                          fillOpacity={(pie.arcs.length - i) / pie.arcs.length}
                        />
                        {hasSpaceForLabel && (
                          <text
                            fill={black}
              ...
              <text style={{ transform: "translate(-15px, 5px)" }}>
                {Math.round(totalRecettes)}K
              </text>
            </Group>
          </svg>
        );
